...
export default function Navbar({ scrolled: externalScrolled }: NavbarProps) {
  ...
  return (
    <>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-12 py-3 md:py-4 flex items-center justify-between">
          ...
          <div className="hidden md:flex gap-1 text-lg font-medium">
            {navLinks.map((link) => (
              ...
              >
                ...
                {/* Hover background */}
                <span
                  className={`absolute inset-0 bg-[#1e40af]/5 dark:bg-white/10 rounded-full transition-all duration-300 ease-out ${hoveredLink === link.href ? 'scale-100 opacity-100' : 'scale-75 opacity-0'
                    }`}
                />

                {/* Underline indicator */}
                <span
                  className={`absolute bottom-0 left-1/2 -translate-x-1/2 h-0.5 bg-gradient-to-r from-[#1e40af] to-[#0d1b49] dark:from-white dark:to-white/60 rounded-full transition-all duration-300 ease-out ${hoveredLink === link.href ? 'w-8' : 'w-0'
                    }`}
                />

                {/* Dot indicator */}
                <span
                  className={`absolute -bottom-1 left-1/2 -translate-x-1/2 w-1 h-1 bg-[#1e40af] dark:bg-white rounded-full transition-all duration-300 ${hoveredLink === link.href ? 'opacity-100 scale-100' : 'opacity-0 scale-0'
                    }`}
                />
              </a>
            ))}
          ...
          <div className="flex items-center gap-2 sm:hidden">
            ...
            <button
              ...
              className={`md:hidden group relative w-11 h-11 flex items-center justify-center rounded-xl transition-all duration-300 active:scale-90 ${mobileMenuOpen
            ...
            >
              <div className="relative w-6 h-5 flex flex-col justify-between">
                <span
                  className={`w-full h-0.5 bg-[#1e40af] dark:bg-white rounded-full transition-all duration-300 origin-center ${mobileMenuOpen ? 'rotate-45 translate-y-[9px]' : ''
                    }`}
                />
                <span
                  className={`w-full h-0.5 bg-[#1e40af] dark:bg-white rounded-full transition-all duration-300 ${mobileMenuOpen ? 'opacity-0 scale-0' : ''
                    }`}
                />
                <span
                  className={`w-full h-0.5 bg-[#1e40af] dark:bg-white rounded-full transition-all duration-300 origin-center ${mobileMenuOpen ? '-rotate-45 -translate-y-[9px]' : ''
      ...
      {/* Mobile Menu Overlay */}
      <div
        className={`fixed inset-0 z-40 bg-black/40 dark:bg-black/60 backdrop-blur-sm transition-all duration-300 md:hidden ${mobileMenuOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'
          }`}
      ...
      {/* Mobile Menu Panel */}
      <div
        className={`fixed top-0 right-0 bottom-0 z-50 w-[85%] max-w-sm bg-white/98 dark:bg-[#0a0f1e]/98 backdrop-blur-2xl shadow-[-20px_0_60px_rgba(0,0,0,0.3)] dark:shadow-[-20px_0_60px_rgba(0,0,0,0.6)] transition-all duration-500 ease-out md:hidden ${mobileMenuOpen
          ? 'translate-x-0'
          : 'translate-x-full'
          }`}
      >
        ...
        <div className="flex flex-col h-[calc(100%-80px)] overflow-y-auto">
          ...
          <div
            className={`px-6 pb-8 pt-4 space-y-3 border-t border-[#1e40af]/10 dark:border-white/10 transition-all duration-500 ${mobileMenuOpen ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'
              }`}
            style={{
